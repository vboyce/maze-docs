...
- **`cumrt`**: ms from the word appearing to the correct press. This includes wrong presses, the delay and the error guard. Only words answered correctly have an entry, so when a `redo: false` trial ends on a mistake, `cumrt` is one shorter than `rt`.
